...
def get_tmdb_link(movie_name, year):
    ...
    for result in search_results:
        if result['release_date'][:4] == str(year):
            return f"https://www.themoviedb.org/movie/{result['id']}"
    return None
...
def get_tvdb_link(series_name, year):
    ...
    for result in search_results:
        if result['firstAired'][:4] == str(year):
            return f"https://www.thetvdb.com/series/{result['id']}"
    return None
...
def main():
    ...
    for movie_name, year, link in movie_items:
        if link:
            print(f"Movie - {movie_name} ({year}): {link}")
        else:
            print(f"Movie - {movie_name} ({year}): Not found")

    for series_name, year, link, missing_seasons in series_items:
        if link:
    ...
    write_to_files(movie_items, 'movies')
    write_to_files(series_items, 'series')
